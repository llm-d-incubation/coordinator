...
# status_text <base_pct> <cur_pct> <threshold>
status_text() {
    local base="$1" cur="$2" threshold="$3"
    awk -v base="$base" -v cur="$cur" -v threshold="$threshold" 'BEGIN {
        if (cur == "" || base == "") { print "missing data"; exit }
        if (threshold > 0 && cur+0 < threshold+0) { print "below threshold"; exit }
        diff = cur - base
        if (diff < -0.05)                         { print "regression"; exit }
        if (diff >  0.05)                         { print "improvement"; exit }
        print "no change"
    }'
}
...
for name in "${all_names[@]}"; do
    ...
    if [[ -n "$base_pct" && -n "$cur_pct" ]]; then
        ...
        status=$(status_text "$base_pct" "$cur_pct" "$THRESHOLD")
        base_fmt="${base_pct}%"
        cur_fmt="${cur_pct}%"
